...
inline void PositionStruct::AddPiece(int sq, int pc) { // 在棋盘上放一枚棋子
	ucsqPieces[pc] = sq;
}
inline void PositionStruct::DelPiece(int sq, int pc) { // 从棋盘上拿走一枚棋子
	ucsqPieces[pc] = 0;
}
		...
		for (int i = RANK_TOP; i <= RANK_BOTTOM; ++i) {
			for (int j = FILE_LEFT; j <= FILE_RIGHT; ++j) {
				...
				if (pc == 0) {
					...
					c = PIECE_BYTE_IN_CHINESE(cnPieceTypes[pc], pc < 32);
					printf("%s", c);
				}
			}
			printf(" %d\n", 12 - i);
