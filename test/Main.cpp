...
int main() {
    ...
    PositionStruct newchess("r3kabnr/9/1c5c1/p1p1p1p1p/9/9/P1P1P1P1P/1C5C1/9/RNBAKABNR r - - 0 1");
    ChessBoard myboard(mychess);
    myboard.DrawBoard(); mychess = newchess;
    PositionStruct chess(mychess);
    ChessBoard mboard(mychess);
    mboard.DrawBoard();
    ...
    memcpy(arr,
        ...
        "RNBAKABNR r - - 0 1",
        70);
    int gotime = 60000;  //给室长的时间存在int k里
    //开始一定为ucci，否则不给开
    PositionStruct mychess;
    while (true) {
    ...
    while (true) {
        ...
        else if (!strncmp(input, "go", 2 * sizeof(char))) {
            // memcpy(tep, input + 8, (MAXLEN - 8) * sizeof(char));
            //if (gettime(tep, MAXLEN, gotime)) 
            {
                //以下应为搜索得出bestmove和走法
