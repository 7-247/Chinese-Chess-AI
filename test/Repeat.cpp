...
int PositionStruct::Repeat() {
    ...
    int h_pos = 0, t_pos = 0;
    for (int i = 0; i < 32; i++) {
        if (ucsqPieces[i] == Moves[length - 1].dst) {
            h_pos = i;
            break;
        }
    }
    for (int i = 0; i < 32; i++) {
        if (ucsqPieces[i] == Moves[length - 2].dst) {
            t_pos = i;
            break;
        }
    }
    if (h_pos != 0 && h_pos != 16 && t_pos != 0 && t_pos != 16) {
        ...
        ucsqPieces[h_pos] = Moves[length - 1].src;  //最后一步回溯
        sdPlayer = !sdPlayer;
        possible = Generate();
        if (readyForKill(sdPlayer, ucsqPieces, possible))
            count[sdPlayer * 16]++;
        ...
        ucsqPieces[t_pos] = Moves[length - 2].src;  //倒数第二步回溯
        sdPlayer = !sdPlayer;
        possible = Generate();
        if (readyForKill(sdPlayer, ucsqPieces, possible))
            count[sdPlayer * 16]++;
        ...
        ucsqPieces[h_pos] = Moves[length - 3].src;  //倒数第三步回溯
        sdPlayer = !sdPlayer;
        possible = Generate();
        if (readyForKill(sdPlayer, ucsqPieces, possible))
            count[sdPlayer * 16]++;
