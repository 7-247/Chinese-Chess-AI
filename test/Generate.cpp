...
static bool exist_friend(int sdPlayer, const int nowPos[],
                         const int chessboard[], int pos, int move) {
    //目标位置是否有友方棋子
    return (chessboard[nowPos[pos] + move] <= 7 + sdPlayer * 7 &&
            chessboard[nowPos[pos] + move] >= 1 + sdPlayer * 7);
}
static bool exist_enemy(int sdPlayer, const int nowPos[],
                        const int chessboard[], int pos, int move) {
    //目标位置是否有敌方棋子
    return (chessboard[nowPos[pos] + move] <= 7 + (!sdPlayer) * 7 &&
            chessboard[nowPos[pos] + move] >= 1 + (!sdPlayer) * 7);
...
static bool chief_rule(int sdPlayer, const int nowPos[],
                       const int chessboard[], int pos, int move) {
    return judge9(sdPlayer, nowPos[pos] + move) &&
           !exist_friend(sdPlayer, nowPos, chessboard, pos, move);
}
...
static bool elephant_rule(int sdPlayer, const int nowPos[],
    ...
    return judgeRiver(sdPlayer, nowPos[pos] + move) &&
           judgeIn(nowPos[pos] + move) &&
           judgeIn(nowPos[pos] + move / 2) &&
           !exist_friend(sdPlayer, nowPos, chessboard, pos, move) &&
...
static bool horse_rule(int sdPlayer, const int nowPos[],
                       const int chessboard[], int pos, int move) {
...
static vector<int> move_horse(int sdPlayer, const int nowPos[],
                              const int chessboard[], int pos) {
    vector<int> possible_move;
    if (judgeIn(nowPos[pos] - 9) &&
        !exist_all(nowPos, chessboard, pos, -9)) {
    ...
    if (judgeIn(nowPos[pos] - 1) && nowPos[pos] % 9 != 1 &&
        nowPos[pos] % 9 != 2 &&
        !exist_all(nowPos, chessboard, pos, -1)) {
...
static vector<int> move_cart(int sdPlayer, const int nowPos[],
    ...
    if (nowPos[pos] % 9 != 1)
        for (int i = -1;; i--) {
            if (judgeIn(nowPos[pos] + i)) {
                if (!exist_friend(sdPlayer, nowPos, chessboard, pos, i))
                    possible_move.push_back(nowPos[pos] + i);
                if (exist_all(nowPos, chessboard, pos, i) ||
                    ((nowPos[pos] + i) % 9 == 1))
                    break;
    ...
    for (int i = -9;; i -= 9) {
        if (judgeIn(nowPos[pos] + i)) {
            if (!exist_friend(sdPlayer, nowPos, chessboard, pos, i))
                possible_move.push_back(nowPos[pos] + i);
            if (exist_all(nowPos, chessboard, pos, i)) break;
        } else
            break;
    }
    for (int i = 9;; i += 9) {
        ...
        for (int i = -1;; i--) {
            if (judgeIn(nowPos[pos] + i)) {
                if (flag) {
                    ...
                    if (exist_all(nowPos, chessboard, pos, i)) {
                        flag = 1;
                        if (exist_enemy(sdPlayer, nowPos, chessboard, pos,
                                        i))
                            possible_move.push_back(nowPos[pos] + i);
                        break;
                    }
    ...
    for (int i = -9;; i -= 9) {
        if (judgeIn(nowPos[pos] + i)) {
            if (flag) {
                ...
                if (exist_all(nowPos, chessboard, pos, i)) {
                    flag = 1;
                    if (exist_enemy(sdPlayer, nowPos, chessboard, pos, i))
                        possible_move.push_back(nowPos[pos] + i);
                    break;
                }
            }
...
static vector<int> move_soldier(int sdPlayer, const int nowPos[],
    ...
    if (judgeRiver(sdPlayer, nowPos[pos])) {
        ...
        if (judgeIn(nowPos[pos] + move) &&
            ...
            possible_move.push_back(nowPos[pos] + move);
        if (judgeIn(nowPos[pos] + 1) && nowPos[pos] % 9 != 0 &&
            !exist_friend(sdPlayer, nowPos, chessboard, pos, 1))
            possible_move.push_back(nowPos[pos] + 1);
        if (judgeIn(nowPos[pos] - 1) && nowPos[pos] % 9 != 1 &&
            !exist_friend(sdPlayer, nowPos, chessboard, pos, -1))
            possible_move.push_back(nowPos[pos] - 1);
...
static inline vector<vector<int>> possibleMove(int sdPlayer,
    ...
    for (int i = sdPlayer * 16; i < sdPlayer * 16 + 16; ++i) {
        if (nowPos[i] != 0)
            possible[i] = move_chess[cnPieceTypes[i]](sdPlayer, nowPos,
                                                      chessboard, i);
    }
...
vector<vector<int>> PositionStruct::Generate() {
    int chessboard[91] = {0};
    for (int i = 0; i < 16; i++)
        chessboard[nowPos[i]] = cnPieceTypes[i] + 1;
    for (int i = 16; i < 32; i++)
